...
final class IOSAppViewModel: ObservableObject {
    ...
    init(
        service: any IOSAppServiceProtocol = IOSAppService(
    ...
    func requestNotifications() {
        if isDemoMode {
            infoMessage = "Notification prompts are disabled in demo mode."
            errorMessage = nil
            return
        }
        Task {
    ...
    func exportDiagnostics() {
        if isDemoMode {
            infoMessage = "Diagnostics export is disabled in demo mode."
            errorMessage = nil
            return
        }
        Task {
    ...
    func loadDiagnosticsPreview() {
        ...
        if isDemoMode {
            diagnosticsPreview = CueDemoContent.shared.diagnosticsPreview()
            errorMessage = nil
            return
        }
    ...
    func enterScreenshotOnboarding() {
        pollingTask?.cancel()
        Task {
            await refreshNotificationPermissionState()
            isCheckingSession = false
            ...
            accountStatus = nil
            scheduledReminders = []
            reminderConfig = ReminderConfigSnapshot(
                enabled: true,
                sevenDayReminder: true,
                threeDayReminder: true,
                oneDayReminder: true,
    ...
    private func updateReminderConfig(_ updated: ReminderConfigSnapshot) {
        if isDemoMode {
            ...
            scheduledReminders = CueDemoContent.shared.scheduledRemindersForSnapshot(snapshot: updated)
    ...
    private func applyDemoState(notificationPermissionState: NotificationPermissionState, infoMessage: String?) {
        isCheckingSession = false
        isAuthenticated = true
        isDemoMode = true
        accountStatus = CueDemoContent.shared.accountStatus()
        reminderConfig = CueDemoContent.shared.reminderConfigSnapshot()
        scheduledReminders = CueDemoContent.shared.scheduledReminders(config: CueDemoContent.shared.reminderConfig())
    ...
    private func leaveDemo(showMessage: Bool) {
        isCheckingSession = false
        isAuthenticated = false
        isDemoMode = false
        accountStatus = nil
        ...
        clearAuthorizationSession()
        errorMessage = nil
        infoMessage = showMessage ? "Exited demo mode." : nil
    }
}
